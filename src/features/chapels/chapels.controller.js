...
const service = require('./chapels.service');

const list = catchAsync(async (req, res) => {
  const { rows, meta } = await service.list(getTenantId(req), req.params.cemeteryId, req.query);
...
const create = catchAsync(async (req, res) => {
  ...
  return created(res, await service.create(getTenantId(req), req.params.cemeteryId, data));
});
